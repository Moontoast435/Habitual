...
class User {
    ...
    static get all() {
        return new Promise(async (resolve, reject) => {
            try {
                let usersData = await pgdb.query(`SELECT * FROM users;`, () => done());
    ...
    static findByUsername(username) {
        return new Promise(async (resolve, reject) => {
            try {
                let userData = await pgdb.query(`SELECT * FROM users WHERE username = $1;`, [username], () => done());
    ...
    destroy() {
        return new Promise(async (resolve, reject) => {
            try {
                const destruction = await pgdb.query(`DELETE FROM users WHERE username = $1 RETURNING username;`, [ this.username ], () => done());
